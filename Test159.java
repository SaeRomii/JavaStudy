   ...
   - java.util.Enumeration<E> 인터페이스
	...
	- java.util.Iterator<E> 인터페이스
	  : Enumeration 과의 차이점은 단지 『remove()』 메소드가 추가된 것 뿐이다.
		...
		정확히 일치하는 기능을 수행한다.
		Enumeration 대신 Iterator를 JDK 1.2부터 추가하여 사용하는 이유는
...
public class Test159
{
	public static void main(String[] args)
	{
		...
		// ① Enumeration
		Enumeration<String> e = v.elements();		//elements메소드 반환
		while (e.hasMoreElements())		//Enumeration이 가지고 있는 메소드를 이용해서 요소 컨트롤 가능
										//hasMoreElements가 true값 반환해서 nextElement로 글자 확인
		{
			System.out.println(e.nextElement());
		}
		//--==>> Java

...
{
	// JDK 1.5(5.0)에서는 인터페이스 메소드를 오버라이딩(Overrideing)할 때
...
public class Test113
{
	...
	{
		//ADemo ob = new ADemo();		//-- 인터페이스는 인스턴스 생성이 불가
		//BDemo ob = new BDemo();		//-- 인터페이스는 인스턴스 생성이 불가

		// ADemo, BDemo 인터페이스를구현(implements)한 클래스(→ DemoImpl)
		// 기반의 인스턴스 생성
		...
		((BDemo)ob2).print();			// DemoImpl이 ADemo, BDemo 둘다 implements 했을때 가능
		((ADemo)ob3).write();
		//--==>> BDemo 인터페이스 메소드...
		//		 ADemo 인터페이스 메소드...

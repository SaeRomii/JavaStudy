...
○ 객체 직렬화의 장점
 ...
 ex) 선생님이 그려주신 커다란 라디오 맨윗칸 lp판 넣는거로 할지 다른거 넣을지 변경 가능한거 예시 생각하기
 
   ※ RMI(Remote Method Invocaton)
      서로 다른 가상 기계장치에 존재하는 함수(메소드)를
	  호출하고 실행하는 기능을 담당한다.
	  서로 다른 통신 구조에 위치한 각각의 객체들 간의
...
○ Serializable 인터페이스(순차적)
   ...
   - 객체 직렬화를 하기 위해 먼저 객체 직렬화가 가능하도록
	 ...
	 형식)
	 ①
	 public class 클래스명 implements Serializable  (추상메소드에서 abstrance 없애려면 오버라이딩 해야하는데 Serializable 쓰면 오버라이딩 안해도됨)
	 {
		// ...
	 }
...
○ Object Stream
   ...
   - java.io.ObjectInputStream
     ObjectInputStream 클래스는 ObjectOutputStream 클래스에 의해			//바이트 기반 스트림 ObjectInputStream, ObjectOutputStream
...
public class Test171
{
	...
	{
		String appDir = System.getProperty("user.dir");		//시스템에서 어떤 디렉토리 사용하는지 반환
		...
		//--==>> false
		// 『C:\JavaStudy\data\test.ser』 요 경로의 파일이 생성되지 않았다는 의미(디렉터리 구성이 존재하지 않음)
		// f0.getParentFile() → C:\JavaStudy\data

		if (!f0.getParentFile().exists())	//-- 디렉터리 구성이 존재하지 않는 상황이 맞다면
		{
			// 디렉터리 경로를 구성하겠다. (만들겠다.)
			f0.getParentFile().mkdirs();	// JavaStudy 위치에 data 폴더 생성됨
		}
		...
		oos.close();				// oos(ObjectOutputStream)의 리소스 반납

		fos.close();				// fos(FileOutputStream)의 리소스 반납

		// 내보내기 끝~

		//---------------------------------------------------------------------------
		// 여기까지 하면 javastudy → data 파일에 test.ser 생김
		...
		if (f0.exists())	//-- f0 파일 객체가 존재한다면
		{
			...
			{
				//key = (String)e.nextElement();		//Object로 구성되어 있어서 String으로
				...
				계속하려면 아무 키나 누르십시오 . . .
				*/
				// JavaStudy 안에 data 폴더 안에 test.ser에서 읽어와서 출력되는거임
			}
		}

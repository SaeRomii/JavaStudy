...
○ 생성자
   - 원형
	 ...
	 → 주어진 문자열 str을 단어로 분리하는 객체를 생성하며
		...
		만약 returnDelim가 true라면... 공백(구분)문자도
		분리된 유효한 데이터로 간주한다.
...
public class Test135
{
	public static void main(String[] args) throws IOException
	{
		...
		//-- 『int countTokens()』 int형을 반환하는 countTokens메소드
		//	  : 파싱된 문자열의 갯수를 리턴(반환)한다.
		//--==>> 파싱할 문자열 입력(컴마구분) : 사과,딸기,바나나,복숭아
		//		 4
		//		 계속하려면 아무 키나 누르십시오 . . .

		strArr = new String[st.countTokens()];	//st.countTokens만큼 방 만들기
		//-- 토큰 분활된 갯수만큼 배열방의 갯수를 구성한다.
